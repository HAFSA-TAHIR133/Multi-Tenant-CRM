/**
 * Emoji picker plugin for Lexical chat editor.
 * Inserts an emoji as plain text at the current selection.
 */
  ...
  return (
    <div className="relative shrink-0">
      <button
        ref={btnRef}
        type="button"
        className={`flex h-[34px] w-[34px] shrink-0 cursor-pointer items-center justify-center rounded-full border-0 text-[1.1rem] leading-none transition-[background-color,transform] duration-150 active:scale-[0.92] ${
          open
            ? "bg-indigo-100 dark:bg-indigo-900"
            : "bg-zinc-100 hover:bg-zinc-200 dark:bg-[#3a3a3c] dark:hover:bg-zinc-600"
        }`}
        title="Add emoji"
        onClick={() => setOpen((v) => !v)}
      >
          ...
          >
            {Object.keys(EMOJI_CATEGORIES).map((cat) => (
              <button
                ...
                className={`flex-1 cursor-pointer rounded-t-md border-0 bg-transparent px-1 py-1.5 text-base leading-none transition-[opacity,background-color] duration-100 ${
                  activeTab === cat
                    ? "bg-zinc-100 opacity-100 dark:bg-[#3a3a3c]"
                    : "opacity-50 hover:bg-zinc-100 hover:opacity-80 dark:hover:bg-[#3a3a3c]"
                }`}
