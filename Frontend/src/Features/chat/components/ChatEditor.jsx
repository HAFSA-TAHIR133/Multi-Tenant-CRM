// ChatEditor.jsx
import {
  $createParagraphNode,
  $createTextNode,
  $getRoot,
} from "lexical";
import { useCallback, useEffect, useRef, useState } from "react";

import { ChatInput } from "./ChatInput";
import { ChatMessage } from "./ChatMessage";
import { chatApi } from "../api/chatApi";

function makeTextState(text) {
  return (editor) => {
    editor.update(() => {
      const paragraph = $createParagraphNode();
      paragraph.append($createTextNode(text));
      $getRoot().append(paragraph);
    });
  };
}

/**
 * ChatEditor shows messages for a specific task.
 * - Only assigned user + admins/superadmins should see/use it (handled at parent).
 * - Each task gets its own ChatEditor (messages scoped by taskId).
 */
export default function ChatEditor({ taskId, currentUser }) {
  const [messages, setMessages] = useState([]);
  const messagesContainerRef = useRef(null);

  const currentRole = currentUser?.role; // 1 user, 2 admin, 3 superadmin

  // Load messages for this task from backend
  useEffect(() => {
    if (!taskId) return;
    let cancelled = false;

    (async () => {
      try {
        const res = await chatApi.getCommentsForTask(taskId);
        const list = Array.isArray(res?.data) ? res.data : Array.isArray(res) ? res : [];
        if (cancelled) return;

        // Map backend messages to ChatMessage format:
        // { id, author, initialState }
        const mapped = list.map((msg) => {
          // You can store text or rich content. For now, assume plain text.
          const text = msg.text || msg.content || "";
          return {
            id: msg.id,
            author: msg.userId === currentUser?.id ? "me" : "buddy",
            initialState: makeTextState(text),
          };
        });

        setMessages(mapped);
      } catch (err) {
        // handle error UI if needed
        console.error("Failed to load chat messages", err);
      }
    })();

    return () => {
      cancelled = true;
    };
  }, [taskId, currentUser]);

  // Scroll to bottom when messages change
  const isInitialMount = useRef(true);
  useEffect(() => {
    if (isInitialMount.current) {
      isInitialMount.current = false;
      return;
    }
    if (messagesContainerRef.current) {
      const container = messagesContainerRef.current;
      container.scrollTop = container.scrollHeight;
    }
  }, [messages]);

  // Submit handler from ChatInput / plugins
  const onSubmit = useCallback(
    (editorState) => {
      // Convert editorState to plain text (or rich JSON if you want).
      let text = "";
      editorState.read(() => {
        text = $getRoot().getTextContent();
      });

      if (!text.trim() || !taskId) return;

      // Persist via API
      chatApi
        .createCommentForTask(taskId, { text })
        .then((res) => {
          const data = res?.data || res;
          const newMsg = {
            id: data.id || Date.now(),
            author: "me",
            initialState: makeTextState(text),
          };
          setMessages((prev) => [...prev, newMsg]);
        })
        .catch((err) => {
          console.error("Failed to send message", err);
        });
    },
    [taskId]
  );

  // Role-based display: only assigned user + admins/superadmins
  // You should compute `canViewChat` in parent using task.assignedUserId and currentUser.
  // For now this component assumes parent has already checked and only renders when allowed.
  if (!taskId) return null;

  return (
    <div className="flex flex-col overflow-hidden rounded-xl border border-solid border-zinc-200 bg-[#f9f9fb] dark:border-white/10 dark:bg-[#1a1a1c]">
      <div className="flex shrink-0 items-center gap-2.5 border-b [border-bottom-style:solid] border-zinc-200 bg-white px-3.5 py-2.5 dark:border-white/[0.08] dark:bg-[#232325]">
        <div className="flex h-[34px] w-[34px] shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#6366f1] to-[#8b5cf6] text-[0.8rem] font-bold text-white select-none">
          C
        </div>
        <div className="flex flex-col gap-px">
          <span className="text-sm leading-tight font-semibold dark:text-zinc-200">
            Task Chat
          </span>
          <span className="text-[0.72rem] leading-tight text-green-500">
            ● Online
          </span>
        </div>
      </div>

      <div
        ref={messagesContainerRef}
        className="flex h-[288px] flex-col gap-1.5 overflow-y-auto px-3.5 pt-3.5 pb-1.5 max-[996px]:h-[228px]"
      >
        {messages.map((msg) => (
          <div
            key={msg.id}
            className={`flex max-w-[78%] items-end gap-1.5 ${
              msg.author === "me" ? "flex-row-reverse self-end" : "self-start"
            }`}
          >
            {msg.author === "buddy" && (
              <div className="mb-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#6366f1] to-[#8b5cf6] text-[0.65rem] font-bold text-white select-none">
                U
              </div>
            )}
            <div
              className={`rounded-2xl px-3 py-2 break-words ${
                msg.author === "buddy"
                  ? "rounded-bl-[4px] bg-white text-[#18181b] shadow-[0_1px_2px_rgba(0,0,0,0.06)] dark:bg-[#2e2e32] dark:text-zinc-200 dark:shadow-none"
                  : "rounded-br-[4px] bg-indigo-500 text-white dark:bg-indigo-600"
              }`}
            >
              <ChatMessage initialState={msg.initialState} />
            </div>
          </div>
        ))}
        {messages.length === 0 && (
          <div className="text-xs text-muted-foreground">
            No messages yet. Start the conversation.
          </div>
        )}
      </div>

      <ChatInput onSubmit={onSubmit} />
    </div>
  );
}